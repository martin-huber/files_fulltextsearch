...
FullTextSearch.prototype = {
    ...
    onEntryGenerated: function (divEntry, entry) {
        ...
        if (entry.info.type !== 'dir') {
            var thumbEntry = divEntry.find('.files_div_thumb');
            thumb = OC.MimeType.getIconUrl(entry.info.mime);
            thumbEntry.css('background-image', 'url("' + thumb + '")');

            OC.Files.getClient().getFileInfo(entry.info.path)
                .then(function (status, resultEntry) {
                    if (resultEntry.hasPreview) {
